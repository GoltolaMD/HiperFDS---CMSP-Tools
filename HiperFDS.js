...
class NotificationManager {
  ...
  injectStyles() {
    ...
    style.textContent = `
      ...
      .notification {
        ...
        margin-bottom: 15px;
        border-radius: 8px;
        box-shadow: 0 4px 12px rgba(22, 2, 2, 0.93);
      ...
      .notification.success {
        border-left-color: #2c682eff;
      }
      .notification.error {
        border-left-color: #cc362bff;
      }
      .notification.info {
        border-left-color: #19619bff;
      }
      .notification::after {
        content: '';
        position: absolute;
        ...
        width: 100%;
        height: 4px;
        background: linear-gradient(90deg, rgba(43, 40, 40, 0.7), rgba(47, 46, 46, 0.7));
      }
      .notification-icon {
        width: 24px;
        height: 24px;
  ...
  getIcon(type) {
    const icons = {
      success: `<svg viewBox="0 0 24 24" fill="#337535ff"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>`,
      error: `<svg viewBox="0 0 24 24" fill="#85261fff"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>`,
      info: `<svg viewBox="0 0 24 24" fill="#2777b9ff"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z"/></svg>`,
    };
...
class ActivityProcessorUI {
  ...
  async processActivities() {
    if (this.isProcessing) {
      this.notificationManager.showNotification(
        "Aviso",
        "O processamento jÃ¡ estÃ¡ em andamentoðŸ¥°.",
        "info",
      );
      return;
    }
    ...
    try {
      let coursePageDom = await this.requestManager
        ...
        .then((response) => {
          if (!response.ok) {
            this.notificationManager.showNotification(
              "ErroâŒ",
              "NÃ£o foi possÃ­vel carregar o curso",
              "error",
            );
      ...
      if (simplePages.length > 0) {
        this.notificationManager.showNotification(
          "Progresso",
          `Marcando ${simplePages.length} atividades como concluÃ­das...`,
          "info",
        );
        await Promise.all(
...
function initActivityProcessor() {
  if (window.location.hostname !== "expansao.educacao.sp.gov.br") {
    ...
    notification.showNotification(
      "ErroâŒ",
      "Este script sÃ³ funciona no site da ExpansÃ£o Educacional de SP",
      "error",
    );
    return;
  }
...
if (document.readyState === "loadingâœ¨") {
